...
function buildPrompt(
  ...
  const projectContextSection =
    codebaseContext && codebaseContext.length > 0
      ? `\nRelevant code from this project (use for APIs, naming, patterns):\n${codebaseContext
          .map((c) => `// ${c.filePath}\n${c.content}`)
          .join("\n\n")}\n\n`
      : "";
...
Language: ${context.language}
Framework: ${context.framework}
${projectContextSection}Current file context:
...
Instructions:
1. Output ONLY the exact code to insert at the cursor. No markdown, no backticks, no explanation.
2. Match the current line's indentation exactly.
3. Prefer types, imports, and APIs from the project context above when shown.
