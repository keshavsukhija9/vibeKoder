...
export const usePlayground = (id: string): UsePlaygroundReturn => {
  ...
  const loadPlayground = useCallback(async () => {
    ...
    try {
      ...
      // Demo playground: no Supabase, load from /api/template/demo
      if (id === "demo") {
        setPlaygroundData({ id: "demo", title: "Demo playground" });
        const res = await fetch("/api/template/demo");
        if (!res.ok) throw new Error(`Failed to load demo: ${res.status}`);
        const templateRes = await res.json();
        const json = templateRes.templateJson;
        setTemplateData(
          json && typeof json === "object" && Array.isArray(json.items)
            ? { folderName: json.folderName ?? "Root", items: json.items }
            : { folderName: "Root", items: [] }
        );
        toast.success("Demo playground ready — try coding! Sign in to save.");
        setIsLoading(false);
        return;
      }
  ...
  const saveTemplateData = useCallback(async (data: TemplateFolder) => {
    if (id === "demo") {
      ...
      toast.info("Demo: changes are in memory. Sign in to save projects.");
      return;
    }
    try {
